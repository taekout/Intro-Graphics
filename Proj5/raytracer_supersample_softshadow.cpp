...
string arg1 = "balls3_transparent.nff";//"balls1_transparent.nff";
string arg2 = "balls3_transparent.ppm";//"balls1_transparent.ppm";
int no_threads = 4;
#endif
...
//cout << *gamma << " " << *beta << " " << 1 - *beta - *gamma << endl;
		...
		if(shader.Transmission > 0)
		{
			...
			if(refractedRay.x != SPACE_END || refractedRay.y != SPACE_END || refractedRay.z != SPACE_END)
				...
				refract_color = TraceRay(interP, refractedRay, air_n, 1, nHitObj, ignoreID, 1); // nInside == 1
		}
...
//	i = [l, r], r = [t,b]
	...
	for(int j = y_begin ; j < y_end ; j++)
	{
		for(int i = 0 ; i < resX ; i++)
		{
			...
			CLIPPING(avgColor.G);
			CLIPPING(avgColor.B);
			unsigned char R = (unsigned char)avgColor.R;
			unsigned char G = (unsigned char)avgColor.G;
			unsigned char B = (unsigned char)avgColor.B;
			pixels[j][i][0] = R;
			pixels[j][i][1] = G;
			pixels[j][i][2] = B;
		}
