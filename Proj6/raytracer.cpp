...
//		return	ret;
	...
	void NormalizeVector()
	{
		...
		if(len == 0)
		{
			cout << "Normal length happened to be 0. This is real rare case. Haha! " << endl;
			exit(-2);
		}
...
public:
	...
	void VectorTurbulence(double rayDir[3], Point &normal, double x, double y, double z)
	{
		double bumpIntensity = 0.2;
		double theta = acos(z / g_ray->m_tmpRadius);
		double phi = atan(y/x);
		double rad = 3.141592;
		phi += 2 * rad;
		double u = phi / (2 * rad);
		double v = ((rad/2) - theta) / (rad/2);
		double noise_x = (cos(x*30 + Noise(u, v, z)*25));
		double noise_y = (cos(y*30 + Noise(v, z, u)*25));
		double noise_z = (cos(z*30 + Noise(z, u, v)*25));
//cout << "before:"; normal.Print();
		Point backupNormal = normal;
		normal.x = (1.0 - bumpIntensity) * normal.x + (bumpIntensity) * noise_x;
		normal.y = (1.0 - bumpIntensity) * normal.y + (bumpIntensity) * noise_y;
		normal.z = (1.0 - bumpIntensity) * normal.z + (bumpIntensity) * noise_z;
		normal.NormalizeVector();
// here if dot(view, surfaceNormal) < 0 ==> recover original normal
		Point dir(rayDir[0], rayDir[1], rayDir[2]);
//		if(Point::DotProduct(dir, normal) < -1)
//			normal = backupNormal;
//cout << "after"; normal.Print();
	}
...
//			exit(-2);
	...
	{
		int nRow = 200 * 2 * radius;
		int nCol = 200 * 2 * radius;
		...
		for(float i = i_min ; i < i_max ; i += 0.01)
		{
			...
			{
				if(int((i-i_min)*100) >= nRow) continue;
				if(int((j-j_min) * 100) >= nCol) continue;
				double tmp;
				if(g_ray ->m_textureKind == 1)
					tmp = cos(i*5 + Noise(i,j,1)*6) ;
				else if(g_ray ->m_textureKind == 2)
					tmp = Turbulence(i,j,1);
				else if(g_ray->m_textureKind == 3)
					tmp = Turbulence(i,j,0);
				else if(g_ray->m_textureKind == 4)
					tmp = cos(i*5 + Noise(i,j,1)*6) ;
	...
	void CreateTextureMap2(float radius)
	{
		...
		for(float i = i_min ; i < i_max ; i += 0.01)
		{
			for(float j = j_min ; j < j_max ; j += 0.01)
			{
				...
				double tmp;
				if(g_ray->m_textureKind == 1)
					tmp = cos(i*3 + Noise(i,j,0)*6) ;//Sum(j*0.01,i*0.01,0);//cos(i + Noise(i,i,j)) ;//Noise(i, j, j);
				else if(g_ray->m_textureKind == 2)
					tmp = Turbulence(i,j,1);
				else if(g_ray->m_textureKind == 3)
					tmp = Turbulence(i,j,0);
...
void Raytracer::DoTexture()
{
	m_file >> m_textureKind;
	if(m_textureKind == 1 || m_textureKind == 2 || m_textureKind == 3 || m_textureKind == 4)
	;
	else
	{
		cout << "Texture Kind must be 1 , 2 or 3 " << endl;
		exit(-1);
	}
}
...
//		float Kd, Ks, PhongPow;
		...
		if(nHitObj == 1)
		{
			...
			u += g_texturemap_size * 1000; v += g_texturemap_size * 1000;
			...
			if(g_ray->m_textureKind == 1)
				...
				for(int k = 0 ; k < 3 ; k++)
					resultColor[k] = g_texture->interpolatedMap[v][u][k];
			else if(g_ray ->m_textureKind == 3)
				{resultColor[0] = 0.7; resultColor[1] = 0.2; resultColor[2] = 0.2;}
//				for(int k = 0 ; k < 3 ; k++)
//					resultColor[k] = g_texture->interpolatedMap[v][u][k];
			else if(g_ray ->m_textureKind == 4)
			{
...
//  texture = cosine( x + perlin(x,y,z) )
			...
			if(m_textureKind == 3)
			{	// bumpy texture
				g_texture->VectorTurbulence(ray_direction, normal, x,y,z);
				normal.NormalizeVector();
			}
